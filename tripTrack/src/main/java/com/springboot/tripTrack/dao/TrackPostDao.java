...
public interface TrackPostDao {
	...
	@Select("select * from tbl_trackpost where trackPost_id = #{user_id}")
	...
	//마이페이지 페이징 테스트
	 @Select(" SELECT * FROM tbl_trackpost WHERE user_id = #{user_id} LIMIT #{limit} OFFSET #{offset}")
	List<TrackPostDto> findPostsByUserId(@Param("user_id") String userId, @Param("limit") int limit, @Param("offset") int offset) throws DataAccessException;

	 @Select("SELECT COUNT(*) FROM tbl_trackpost WHERE user_id = #{user_id}")
	 int countPostsByUserId(@Param("user_id") String userId) throws DataAccessException;
//////////////////////////////////////////////////////////////////////////////////////////////////////////
